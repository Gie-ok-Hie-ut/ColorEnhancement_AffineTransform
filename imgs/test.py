...
class BLTSmoothnessLoss(nn.Module):
    def __init__(self, height, width, sigma_s = 1, sigma_r = 0.1):
        ...
        self.pad = nn.ReflectionPad2d(self.fr)
        self.height = height
        self.width = width
    ...
    def forward(self, input):
        ...
        for k in range(0, self.k_size): # height
            for l in range(0, self.k_size): # width
                shifted_input = padded_input.narrow(2, k, self.height).narrow(3, l, self.width)
                if k == 0 and l == 0:
                    self.loss_map = (input - shifted_input).abs() * weight_map[widx].repeat(1, input.size(1), 1, 1)
                else:
                    self.loss_map += (input - shifted_input).abs() * weight_map[widx].repeat(1, input.size(1), 1, 1)
                widx += 1
...
img = Image.open('high.jpg').convert('RGB')
...
Image.fromarray(input_np).save('img_cropped.png')

res_np = tensor2im(res.data)
Image.fromarray(res_np).save('img_enhanced.png')
...
# tmap = netG.transform.data[0][0].view(height, width).cpu().numpy()*255
# Image.fromarray(tmap.astype(np.uint8), mode='L').save('mapL_1.jpg')
# tmap = netG.transform.data[0][1].view(height, width).cpu().numpy()*255
# Image.fromarray(tmap.astype(np.uint8), mode='L').save('mapL_2.jpg')
# tmap = netG.transform.data[0][2].view(height, width).cpu().numpy()*255
# Image.fromarray(tmap.astype(np.uint8), mode='L').save('mapL_3.jpg')
# tmap = netG.transform.data[0][3].view(height, width).cpu().numpy()*255
...
# Image.fromarray((layer.loss_map.data[0][1].cpu().numpy()*10*255).astype(np.uint8), mode='L').save('loss.jpg')